...
Strata ships with a Pi extension that gives Pi coding agents built-in memory awareness. It does three things: injects Strata context into the agent's system prompt, captures full conversation transcripts after every prompt, and  -  via a background daemon  -  extracts standalone knowledge files from those transcripts. Not bad for a single TypeScript file.

## Overview

The Strata Pi extension lives at `skills/pi/strata.ts` and implements the [Pi Extension API](https://github.com/earendil-works/pi-coding-agent). It provides two hooks:

1. **`before_agent_start`**: Injects Strata architecture and commands into the agent's system prompt at session start. The agent learns about the three tiers and key CLI commands without you having to explain them every time.
2. **`agent_end`**: After each prompt completes, saves the full conversation transcript (user + assistant messages) as a plain markdown file. No heuristic filtering  -  the raw transcript is always preserved. We don't try to guess what's important.
...
Once the extension is installed and Pi is capturing conversations, you're two steps away from LLM-powered fact extraction.
...
At session start, the extension appends a Strata memory section to the agent's system prompt. The agent doesn't need to read a manual  -  it just knows.
...
After each prompt completes, the extension saves the full conversation (all user and assistant messages) as a plain markdown file. Unlike the previous `turn_end` handler, there is **no heuristic filtering**  -  the raw transcript is always preserved. We tried being clever about what to keep. It didn't work.
...
**Writing logic:** Uses `node:fs` to write directly to the Strata directory (no `strata add` CLI call), with `recursive: true` directory creation. Filesystem errors are silently caught  -  the extension never blocks Pi. Pi has better things to do.
...
The configuration file is **optional**  -  without it, the daemon skips distillation but the extension still captures raw transcripts. Conversation capture always works; extraction is the nice-to-have.
...
| `llm.temperature` | `0.0` | LLM temperature (0.0 = deterministic extraction). No creativity needed for "what were we talking about." |
...
When `pi-config.json` has `llm.enabled: true` and a valid API key, the Strata daemon performs background distillation as the first step of each maintenance cycle. This runs every 15 minutes by default (`strata serve`). Your conversations get digested while you're off doing something else.
...
2. **Batch**: All new transcripts are joined together and sent to the configured LLM in a single API call. Economy of scale.
...
The daemon's first cycle is always a dry run (unless `--live` is passed). During the dry run, distillation reports how many transcripts would be processed without calling the LLM or writing files. It's like a dress rehearsal, but with less choreography.

### Cost

Distillation calls the LLM once per maintenance cycle (every ~15 minutes), in a single batch. At GPT-4o-mini pricing (~$0.15/1M input tokens), a typical cycle costs less than $0.01. No API call is made when there are no new undistilled transcripts. You'll spend more on coffee than on this.
...
- If the LLM returns "no significant facts to extract," conversations are still marked as processed  -  no fact file is written. Sometimes there's just nothing noteworthy.
...
The agent can still use `strata search` to find relevant context. Distillation enriches the fact corpus over time, making searches more productive across sessions. Think of it as compound interest for your memory.
...
For non-Pi agents (OpenCode, Claude Code, Cursor, Codex, Windsurf  -  the list keeps growing), Strata provides a skill install via the Vercel Skills protocol:
...
This installs a skill definition (SKILL.md) that teaches the agent Strata's architecture and commands. Unlike the Pi extension, the skill is static  -  it provides knowledge but does not run hooks. No background daemons, no transcript capture. It's the "tell, don't show" approach.

## The Extension Code

The extension source (`skills/pi/strata.ts`) is deliberately zero-dependency and runs in Pi's TypeScript runtime. Here's what we optimized for:

- **Always-on conversation capture.** The `agent_end` hook saves every conversation transcript with no heuristic filtering  -  raw data is never discarded. Better to have it and not need it.
- **Zero npm dependencies.** Uses Node.js builtins (`node:fs`, `node:crypto`, `node:path`, `node:os`) and globally available `fetch()` (Node 18+) for API calls. No `package.json` needed. No supply chain drama.
...
- **Silent failures.** If the filesystem is unwritable, the extension silently skips. It never blocks Pi. Your coding session > our error messages.
...
Every conversation is captured as a transcript. Over time, the daemon extracts standalone facts from those transcripts into `pi/facts/`. The agent uses `strata search` to find relevant context  -  searches across transcripts (1st Stratum), facts, memos, cooled files, and archived content. Your memory gets better the more you use it.
...
The extension, distiller, and daemon are complementary. The extension captures raw data. The distiller extracts knowledge. The daemon manages the lifecycle across all strata. Each one minds its own business.

## Migration from Previous Versions

Prior to this version, the extension used a `turn_end` handler with an optional LLM classifier and heuristic fallback. It was... let's say "creative" about which conversations it kept. Here's what changed:
...
Existing `pi/memos/` files are untouched. New captures go to `pi/conversations/`. Nobody's deleting your old stuff.
